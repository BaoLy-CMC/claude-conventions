...
4. **Convention reviews** on the diff — invoke `logging-review` and `error-handling-review` (and `security-review` / `api-versioning-check` / `idempotency-review` if installed). Collect their CRITICAL/HIGH findings.
